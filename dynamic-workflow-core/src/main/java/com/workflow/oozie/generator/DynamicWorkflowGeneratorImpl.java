...
public class DynamicWorkflowGeneratorImpl implements WorkflowGenerator {
	...
	public void generateWorkFlow(DynamicWorkflowConfig dynamicWorkflowConfigObj) throws IOException {
		if (dynamicWorkflowConfigObj != null) {
			...
			if (dynamicWorkflowConfigObj.getGlobalNodeDetails() != null) {
				workFlowApp.getDecisionOrForkOrJoin().add(
						nodeCreator.createGlobalNode(dynamicWorkflowConfigObj.getGlobalNodeDetails()));
			}
		}
		 ...
		 * "ssh-command", Arrays.asList(args), "ssh-ok-name",
		 * "ssh-error-name"));
		 */
		...
		try {
			...
			File file = new File("C:\\Users\\Asif\\Desktop\\myWorkflow");
